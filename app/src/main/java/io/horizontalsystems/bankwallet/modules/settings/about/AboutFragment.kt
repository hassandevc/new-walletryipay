...
private fun SettingSections(viewModel: AboutViewModel, navController: NavController) {
    ...
    CellSingleLineLawrenceSection(
        listOf({
            ...
            HsSettingCell(
                    R.string.SettingsAboutApp_Telegram,
                    R.drawable.ic_telegram_20,
                    onClick = { LinkHelper.openLinkInAppBrowser(context, viewModel.telegramLink) }
            )
        },{
            HsSettingCell(
...
fun AboutHeader(appVersion: String) {
    Row(
        ...
        Image(
            ...
            painter = painterResource(id = R.drawable.logoryii),
            contentDescription = null,
        )
        Column(
